<#
	...
	.NOTES
		=================================================================================
		 ...
		 Minor fixes by: Tom Riddle
		 	-sql native client downloads in pwsh7
			-$SkipAutoReboot added while preserving all other $AssumeYes conditions
		=================================================================================
#>
param
(
	...
	[Parameter(HelpMessage = 'Wont reboot the server')]
	[Alias('sar')]
	[switch]$SkipAutoReboot
)
...
function Start-SCOMTLSEnforcement
{
	...
	{
		#This breaks when using remote nas paths like \\nas\share\path
		$resolvedPath = Resolve-Path $DirectoryForPrerequisites
		if($DirectoryForPrerequisites -notlike "\\*"){
			$DirectoryForPrerequisites = $resolvedPath.Path
		}
	...
	if (-not $SkipDownloadPrerequisites -and $missingPreq -or $ForceDownloadPrerequisites)
	{
		...
		function Download-Prerequisites
		{
			...
			try
			{
				...
				$basepage = Invoke-WebRequest -Uri 'https://www.microsoft.com/download/details.aspx?id=50402&751be11f-ede8-5a0c-058c-2ee190a24fa6'
				$scriptcontents=[regex]::Match($basepage.RawContent,'<script>(.*"Microsoft® SQL Server® 2012 Native Client - Latest Servicing Release".*)</script>')

				$scriptcontentsNoGreed = $scriptcontents.Groups[1].value -split "</script>" | select -first 1
				$jsonstr = $scriptcontentsNoGreed -split "^.+?=" | select -last 1
				$jsonobj = $jsonstr  | ConvertFrom-Json
				
				$x64Fileinfo = $jsonobj.dlcDetailsView.downloadFile | ?{$_.name -like "*x64*"}
				$x86Fileinfo = $jsonobj.dlcDetailsView.downloadFile | ?{$_.name -like "*x86*"}

				$releaseVersion = $x64Fileinfo.version
				$releaseDate = $x64Fileinfo.datePublished.ToShortDateString().Replace("/", "-")
				$releaseDownloadLink = $x64Fileinfo.url
...
#>
	...
	IF (-not $SkipSQLSoftwarePrerequisites -and ($ManagementServer -or $WebConsoleServer) -or $IsSQLServer)
	{
		...
		if ($SCOM2012 -or $SCOM2016 -and -not $MSOLEDBGood)
		{
			...
			Write-ScriptLog -Step 'Check SQL Prerequisites' -Status Passed -LogString "SQL Client - Only needed on SCOM 2012/2016 " -ForegroundColor Green
		}
	...
	IF ($Answer -match "^Y" -and $SkipAutoReboot -ne $true)
	{
...
if ($AssumeYes -or $SkipDotNetCheck -or $SkipModifyRegistry -or $SkipRoleCheck -or $SkipSQLQueries -or $SkipSQLSoftwarePrerequisites -or $SkipVersionCheck -or $SkipDownloadPrerequisites -or $DirectoryForPrerequisites -or $ForceDownloadPrerequisites -or $SkipAutoReboot)
{
	Start-SCOMTLSEnforcement -AssumeYes:$AssumeYes -ForceDownloadPrerequisites:$ForceDownloadPrerequisites -SkipDotNetCheck:$SkipDotNetCheck -SkipModifyRegistry:$SkipModifyRegistry -SkipRoleCheck:$SkipRoleCheck -SkipSQLQueries:$SkipSQLQueries -SkipSQLSoftwarePrerequisites:$SkipSQLSoftwarePrerequisites -SkipVersionCheck:$SkipVersionCheck -SkipDownloadPrerequisites:$SkipDownloadPrerequisites -DirectoryForPrerequisites:$DirectoryForPrerequisites -SkipAutoReboot:$SkipAutoReboot
}
else
{
